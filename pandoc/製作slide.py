from streamlit import spinner as stSpinner, set_page_config as pageConfig, title, subheader, text_input, multiselect, button, warning, success, download_button, error#, set_page_config, write as stWrite,
...
def resize_image(url):
    ...
    image = imgOpen(requests.get(url, stream=True).raw)    # Open the image file
...
#set page layout
pageConfig(page_icon="🚀", page_title="PowerPoint Generator")
...
title('Welcome to the PowerPoint Generator!')

subheader('This web app uses data from Yahoo Finance to create PowerPoint slides')

user_input = text_input(label='Enter company ticker. For example: AAPL for Apple or TSLA for Tesla')

options = ['Stock Price', 'Revenue', 'Market Cap', 'EBITDA']
selected_options = multiselect(label='Select metric(s)', options=options)

submit = button(label='Generate PowerPoint slides')

# trim user input string
user_input = str(user_input.lower()).strip()

if submit and user_input == "":
    warning("Please enter company ticker!")

elif submit and user_input != "":
    from pathlib import Path
    with stSpinner('Generating awesome slides for you...⏳'):
        try:
            # open presentation template
            pptx = Path(path)/'template.pptx'
            ...
            success('The slides have been generated! :tada:')

            download_button(label='Click to download PowerPoint', data=binary_output.getvalue(), file_name=filename)

        except Exception as e:
            rndrCode(e)
            error("Oops, something went wrong! Try again or use a different ticker")
