...
pandoc -f markdown-raw_tex --pdf-engine=xelatex -V CJKmainfont=華康魏碑體 -o a.pdf /home/josh/nhird/MDs/markdown-sample.md  -V 'mainfont:URW Gothic'

轉換至 PDF 超連結出現沒有顏色 Pandoc conversion to PDF not providing colored hyptertext links

Consider the following snippet of a PDF generated from pandoc by way of latex. Were you able to identify the hypertext links in there? Neither was I... It turns out that the second Kaplan Meier has a fully functional link to an external site. But how do we divine that?Note that the following option is already in place in the pandoc preamble - without which the hyperlink would not work at all: link-citations: true
Short of a "real" solution I'll need to hack something to make hints for the links - possibly via font manipulations. pandoc pandoc-citeproc 
If you use a recent version of Pandoc (latest is v2.7.3), just add (for example) these to the commandline:
-V colorlinks=true \
-V linkcolor=blue \
-V urlcolor=red \
-V toccolor=gray
No need to fiddle with the preamble You can investigate the built-in LaTeX template shipping with your own Pandoc by this command:
...
pandoc -s -o output.html input.txt For more information on how standalone documents are produced, see Templates below.
